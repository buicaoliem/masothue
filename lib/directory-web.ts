...
export async function listProfilesFiltered(
  groupSlug: string,
  provinceSlug: string,
  page: number,
  ...
  return prisma.$queryRawUnsafe<PublicProfile[]>(
    ...
    provinceSlug,
    DIRECTORY_PAGE_SIZE,
    (p - 1) * DIRECTORY_PAGE_SIZE,
  );
}

export async function countProfilesFiltered(groupSlug: string, provinceSlug: string, filter: ProfileFilter): Promise<number> {
...
export async function getGroupCountsNationwide(): Promise<Map<string, number>> {
  const rows = await prisma.$queryRawUnsafe<{ groupSlug: string; n: bigint }[]>(
...
export async function getProvinceSlugsWithAnyProfile(): Promise<string[]> {
  const rows = await prisma.$queryRawUnsafe<{ provinceSlug: string }[]>(
...
export async function getGroupSlugsForProvince(provinceSlug: string): Promise<string[]> {
  const rows = await prisma.$queryRawUnsafe<{ groupSlug: string }[]>(
...
export async function getGroupCountsForProvince(provinceSlug: string): Promise<Map<string, number>> {
  const rows = await prisma.$queryRawUnsafe<{ groupSlug: string; n: bigint }[]>(
    `SELECT p.group_slug AS "groupSlug", count(*) AS n FROM business_profile p
...
export async function getProvinceSlugsForGroup(groupSlug: string): Promise<string[]> {
  const rows = await prisma.$queryRawUnsafe<{ provinceSlug: string }[]>(
...
export async function getSameGroupProfiles(
  groupSlug: string,
  provinceSlug: string,
  excludeMst: string,
  limit = 6,
...
export async function getIndexableDirectoryPaths(): Promise<{ groupSlug: string; provinceSlug: string }[]> {
  return prisma.$queryRawUnsafe<{ groupSlug: string; provinceSlug: string }[]>(
